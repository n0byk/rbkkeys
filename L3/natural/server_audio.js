...
let interval = setInterval((_) => {
  let dirs = fs.readdirSync("../data/audio_texts/");
  dirs.forEach((_) => {
...
function stem(name) {
  ...
  let obj = JSON.parse(fs.readFileSync("../data/audio_texts/" + name));
...
function saveFile(str, morfology) {
  const words = wordsCount(str);
  let res = JSON.stringify({ words, txt: str, morfology }, 2);
  let file_name = current_dir.split(".")[0];
  let dir = "../data/stemmed_texts/" + file_name;
  console.log("dir = ", dir, "file_name=", file_name);
  createDir(dir + "/");
  fs.writeFileSync(dir + "/audio_" + file_name + ".json", res);
}
